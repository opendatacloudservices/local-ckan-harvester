...
import {logError, localTokens} from '@opendatacloudservices/local-logger';
...
export const resetState = (
  ...
  return client
    .query(
      `UPDATE ${prefix}_packages SET ckan_status = TO_JSON(STRING_TO_ARRAY(ckan_status, '_'))->>-1 WHERE id = $1`,
      [id]
    )
    .then(() => {});
};

export const deprecatePackages = (
  client: Client,
  prefix: string
): Promise<void> => {
  return client
    .query(
      `UPDATE ${prefix}_packages SET ckan_status = CONCAT('obsolete_', ckan_status)`
    )
    .then(() => {});
};

export const setPackageStatus = (
  client: Client,
  prefix: string,
  id: string,
  status: string
...
export const processPackage = (
  ...
  return packageGetAction(client, prefix, ckanPackage).then(
    async (action: string) => {
      if (action === 'nothing') {
        await resetState(client, prefix, ckanPackage.result.id);
...
export const getInstance = (
  ...
  return client
    .query(
      `SELECT id, prefix, domain, version, rate_limit FROM ${definition_master_table} WHERE ${
